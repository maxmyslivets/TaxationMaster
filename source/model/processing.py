...
class Processing(QtCore.QObject):
    ...
    def create_taxation_plan(self, dxf_path: Path, numbers_layers: list[str], lines_layers: list[str],
        ...
        for k_split_number, split_number in taxation_plan.split_numbers.items():
            for k_number in taxation_plan.number_from_split_number[k_split_number]:
                # print(k_number)
                taxation_plan_object = [split_number, taxation_plan.numbers[k_number]]
        ...
        for k_number, source_number in numbers.items():
            ...
            for number in items:
                ...
                if re.match(r'^\d+$', number):
                    ...
                    number_from_split_number[k_split_number] = [k_number]
                ...
                # Извлечение диапазона чисел с разделителем "-"
                    ...
                    for new_number in result:
                        k_split_number += 1
                        split_numbers[k_split_number] = new_number
                        if k_split_number not in number_from_split_number:
                            number_from_split_number[k_split_number] = []
