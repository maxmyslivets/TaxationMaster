...
class Config:
    ...
    def default_setup(self) -> None:
        ...
        # temp_path
        self.config["temp_path"] = str(Path(tempfile.gettempdir()) / "TaxationTool")
        # converter
        self.config["temp_path_convert_input"] = str(Path(self.config["temp_path"]) / "convert" / "input")
        self.config["temp_path_convert_output"] = str(Path(self.config["temp_path"]) / "convert" / "output")
        self.config["oda_converter_path"] = str(_get_system32() / "ODAFileConverter.exe")
        self.config["timeout"] = 10
        # taxation_plan
        self.config["numbers_layers"] = ["номера"]
        self.config["lines_layers"] = ["полосы"]
        self.config["contours_layers"] = ["контуры"]
        self.config["zones_layers"] = ["зоны"]
        self.config["min_distance"] = 0.01
        self.config["min_area"] = 0.01
        self.save()

    @property
    def temp_path(self) -> Path:
        return Path(self.config["temp_path"])

    @temp_path.setter
    def temp_path(self, path: str | Path) -> None:
        self.config["temp_path"] = path if isinstance(path, str) else str(path)

    @property
    def temp_path_convert_input(self) -> Path:
        return Path(self.config["temp_path_convert_input"])

    @temp_path_convert_input.setter
    def temp_path_convert_input(self, path: str | Path) -> None:
        self.config["temp_path_convert_input"] = path if isinstance(path, str) else str(path)

    @property
    def temp_path_convert_output(self) -> Path:
        return Path(self.config["temp_path_convert_output"])

    @temp_path_convert_output.setter
    def temp_path_convert_output(self, path: str | Path) -> None:
        self.config["temp_path_convert_output"] = path if isinstance(path, str) else str(path)

    @property
    def oda_converter_path(self) -> Path:
        return Path(self.config["oda_converter_path"])

    @oda_converter_path.setter
    def oda_converter_path(self, path: str | Path) -> None:
        self.config["oda_converter_path"] = path if isinstance(path, str) else str(path)
    ...
    def min_area(self) -> float:
        return float(self.config["min_area"])
